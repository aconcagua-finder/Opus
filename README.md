...
- ИИ-импорт: «Добавить список слов (ИИ)» анализирует текст до 2 МБ, поддерживает режим «Определять фразы» и добавляет подсказку-пример (до 1500 символов) с полноценным переводом примера
- Промпты и настройки словарного ИИ вынесены в `features/dictionary/prompts/ai-import.ts` для быстрой корректировки без правки API-слоя
