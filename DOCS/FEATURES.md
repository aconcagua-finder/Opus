...
- ✅ Панель фильтров сворачивается, состояние сохраняется в настройках пользователя
  ...
  - ✅ Редактирование записей и сохранение последних выбранных языков в форме
- ✅ Импорт с помощью ИИ: анализ текста до 2 МБ, переключатель «Определять фразы» и автоматическая подсказка-пример (до 1500 символов) с полным переводом примера
- **Особенности**:
  ...
  - Все промпты словарного ИИ вынесены в `features/dictionary/prompts/ai-import.ts`
  - Пользовательские настройки ИИ живут в zustand-сторе `useDictionaryAiSettingsStore` и сохраняются в `localStorage`
...
- **Компоненты**:
  ...
  - ✅ Textarea - многострочные поля с общим стилем
  - ✅ Card - карточки с Header, Content, Footer
  - ✅ Alert - уведомления об ошибках
- **Особенности**:
  - Светлая и тёмная темы + авто-режим
  - Переключатель темы (light / system / dark)
  - Cyan акценты
...
- **Реализовано**:
  ...
  - ✅ `/dictionary` - защищенная страница словаря
  - ✅ `/settings` - защищенная страница настроек ИИ и аккаунта
  - ✅ `/dashboard` - временный редирект на словарь (для обратной совместимости)
- **Особенности**:
